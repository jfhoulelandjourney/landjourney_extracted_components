...
/**
 * Inspector panel for name fields.
 *
 * Identical structure to the date inspector — name is also an
 * **optional-assignment** field. Leaving `signee` null is valid and does NOT
 * trigger the orphan indicator in the overlay. The user can still assign a
 * signee if they want to gate the name field to a specific participant.
 */
@Component({
  selector: 'lj-pdf-name-inspector',
  standalone: true,
  imports: [
    ActivateDirective,
    CommonModule,
  ...
  styleUrl: './name.inspector.scss',
})
export class NameInspectorComponent extends BaseFieldInspector<'name'> {
