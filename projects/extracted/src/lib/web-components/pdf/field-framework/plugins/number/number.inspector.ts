...
import type { NumberFieldData } from './number.plugin';

/**
 * Inspector panel for number fields.
 *
 * Provides configuration for: label, placeholder, min/max constraints,
 * decimal precision, allow negative flag, and optional-assignment signee/signer.
 */
@Component({
  selector: 'lj-pdf-number-inspector',
  standalone: true,
  imports: [
    ActivateDirective,
    CommonModule,
  ...
  templateUrl: './number.inspector.html',
  styleUrl: './number.inspector.scss',
})
export class NumberInspectorComponent extends BaseFieldInspector<'number'> {
  ...
  /** Local signals for form inputs — independent of PSPDFKit data to prevent focus loss. */
  ...
  constructor() {
    ...
    effect((): void => {
      const data = this.data() as NumberFieldData | null;
      if (data) {
