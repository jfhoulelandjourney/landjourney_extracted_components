...
import type { CheckboxFieldData, CheckboxOption } from './checkbox.plugin';

let optionCounter = 0;

function makeNewOption(): CheckboxOption {
  return {
...
/**
 * Inspector panel for checkbox fields.
 *
 * Mirrors the dropdown inspector. Per-option fields are `value` + `label`,
 * plus a `checked` toggle marking the default-checked state at fill time.
 * Independent toggles — multiple options can be checked simultaneously.
 */
@Component({
  selector: 'lj-pdf-checkbox-inspector',
  standalone: true,
  imports: [
    ActivateDirective,
    CommonModule,
  ...
  templateUrl: './checkbox.inspector.html',
  styleUrl: './checkbox.inspector.scss',
})
export class CheckboxInspectorComponent extends BaseFieldInspector<'checkbox'> {
  ...
  readonly optionsInput = signal<readonly CheckboxOption[]>([]);
  ...
  /**
   * See dropdown.inspector.ts:139 — same race-condition rationale: only seed
   * `optionsInput` from external data on field-switch, not on every patch
   * round-trip, so per-keystroke patches don't blow away the user's typing.
   */
  ...
  constructor() {
    ...
    effect((): void => {
      const data = this.data() as CheckboxFieldData | null;
  ...
  trackOptionById(_index: number, option: CheckboxOption): string {
    return option.id;
  }
  ...
  /**
   * Toggle "default checked" on an option. Independent toggles — multiple
   * checkboxes may be default-checked at the same time.
   */
  onOptionCheckedChange(index: number, checked: boolean): void {
    const current = this.optionsInput();
    const next = current.map((o, i) => (i === index ? { ...o, checked } : o));
    this.optionsInput.set(next);
    void this.patch({ options: next });
  }
