...
@Component({
  selector: 'lj-pdf-initials-inspector',
  standalone: true,
  imports: [
    ActivateDirective,
    CommonModule,
  ...
  templateUrl: './initials.inspector.html',
  styleUrl: './initials.inspector.scss',
})
export class InitialsInspectorComponent extends BaseFieldInspector<'initials'> {
  readonly type = 'initials' as const;
  ...
  constructor() {
    ...
    // Sync label input from PSPDFKit data when the annotation selection changes.
    effect((): void => {
  ...
  async confirmSigneeCreate(): Promise<void> {
    // Synchronous re-entry guard: signal reads/writes are sync and current
    // within a single click handler, so two rapid clicks → first sets true,
    // second reads true and bails.
