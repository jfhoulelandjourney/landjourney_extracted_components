...
/**
 * Inspector panel for text-input fields.
 *
 * Same structure as the name and date inspectors — text-input is also an
 * **optional-assignment** field. Leaving `signee` null is valid and does NOT
 * trigger the orphan indicator. Required defaults to `false` (vs `true` on
 * name) — text-input is a generic text box and most uses do not gate submit
 * on it.
 */
@Component({
  selector: 'lj-pdf-text-input-inspector',
  standalone: true,
  imports: [
    ActivateDirective,
    CommonModule,
...
export class TextInputInspectorComponent extends BaseFieldInspector<'text-input'> {
  readonly type = 'text-input' as const;
