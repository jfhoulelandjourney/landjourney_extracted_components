...
@Component({
  selector: 'lj-credit-line-document-preview',
  templateUrl: './credit-line-document-preview.component.html',
  styleUrls: ['./credit-line-document-preview.component.scss'],
  imports: [
    MatIconModule,
    ActivateDirective,
    LjImageComponent,
...
export class CreditLineDocumentPreviewComponent implements AfterViewInit {
  private router = inject(Router);
  private organizationService = inject(OrganizationService);
  private clientDocumentsService = inject(ClientDocumentsService);

  creditLineId = input.required<string>();
  creditLineUsers = input.required<LoanUserBaseSchema[]>();
  ...
  goToDocument() {
    if (this.isMobile()) {
      this.router.navigateByUrl(
        `/tabs/loans/credit/${this.creditLineId()}/documents/${this.attachment()?.id}`
      );
    } else {
      this.router.navigateByUrl(
        `/loans/credit/${this.creditLineId()}/documents/${this.attachment()?.id}`
      );
    }
  }
  ...
  getUserName() {
    for (const event of this.attachment()?.history ?? []) {
      if (event.action === LendAttachmentHistoryActions.CREATED) {
        ...
        for (const user of this.creditLineUsers()) {
    ...
    return 'SYSTEM'; // THIS SHOULD RETURN THE LOAN OFFICER NAME BUT THE LOAN OFFICER IS NOT PART OF USERS ON CREDIT LINE
  }
}
