...
export class AttachmentsService {
  ...
  unregisterAttachment(fileId: string, digest: string): Observable<undefined> {
    return this.documentService
      .deleteFileMetadata(fileId, digest)
  ...
  uploadFileToAttachment(
    ...
    const metadata = this.createFileMetadata(file, {
      ...
      fileMetadata: {
        ...props.fileMetadata,
        originalName: file.name ?? props.fileName,
      },
    });
  ...
  /**
   * Create a new attachment with the provided blob and metadata
   */
  createAttachmentWithBlob(
    file: Blob,
    attachmentMetadata: AttachmentMetadata,
    props: {
      fileName?: string;
    ...
    return this.uploadFileToAttachment(file as File, attachmentMetadata, props);
  }

  getAttachmentFileInfo(
    attachment: Attachment
  ): Observable<ExistingFileMetadata | null> {
    const { documentId, digest } = attachment;
  ...
  updatePdfMetadata(file: ExistingFileMetadata): Observable<ApiMessage> {
    return this.documentService.patchPdfMetadata(
      file.id,
      file.digest,
      file.fileMetadata ?? {}
    );
